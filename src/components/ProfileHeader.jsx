...
import { User, Pencil, Flame } from 'lucide-react';
...
export default function ProfileHeader({ name, role, streak, avatarUrl, onEdit }) {
  ...
  return (
    <div className="flex items-center justify-between p-5 md:p-6 rounded-2xl bg-white/5 backdrop-blur border border-white/10">
      <div className="flex items-center gap-4 md:gap-5">
        ...
        <div>
          <div className="flex items-center gap-3">
            <h2 className="text-lg md:text-xl font-semibold text-white">{name}</h2>
            <div className="inline-flex items-center gap-1.5 px-2 py-1 rounded-full bg-amber-400/10 text-amber-300 border border-amber-300/20">
      ...
      <button
        onClick={onEdit}
        className="inline-flex items-center gap-2 px-3.5 py-2 rounded-xl bg-white/10 hover:bg-white/15 border border-white/15 text-white transition"
      >
