...
const ProfileHeader = ({ name, role, streak, avatarUrl, onEdit }) => {
  return (
    <section className="w-full rounded-2xl bg-white/70 backdrop-blur border border-gray-200 p-6 shadow-sm">
      <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
        <div className="relative">
          {avatarUrl ? (
            <img
              src={avatarUrl}
              alt={`${name}'s avatar`}
              className="h-20 w-20 rounded-full object-cover border border-white/60 shadow"
            />
          ) : (
        ...
        <div className="flex-1 text-center sm:text-left">
          <h1 className="text-2xl font-semibold text-gray-900 flex items-center justify-center sm:justify-start gap-3">
            {name}
          </h1>
